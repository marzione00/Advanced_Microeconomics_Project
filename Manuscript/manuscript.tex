...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.45\textwidth}
         ...
         \caption{Lazy PM}
     \end{subfigure}
\caption{Redundancy funds variation (in terms of million of hours paid) for the scenarios where an active vs. lazy PM is considered. These effects were calculated by running a DID regression for the different national lock-down imposed during March-June 2020 and then multiplying the number of lock-down weeks obtained from the SIRD scenario with the coefficients obtained from the DID (scaled in order to obtain the week value). The standard error bars, although they are not clearly visible because they are too reduced,  were calculated by considering the error propagation}
...
\begin{table}[]
\begin{center}
\caption{Sales value effects for the scenarios where an active vs lazy PM is considered (with the baseline $100=2015$).  These effects were calculated by running a DID regression for the different national lock-down imposed during March-June 2020 and then multiplying the number of lock-down weeks obtained from the SIRD scenario with the coefficients obtained from the DID (divided by 4).  The standard errors were calculated by considering the error propagation}
...
\begin{table}[]
\begin{center}
\caption{Redundancy funds variation (in terms of million of hours paid) for the scenarios where an active vs. lazy PM is considered. These effects were calculated by running a DID regression for the different national lock-down imposed during March-June 2020 and then multiplying the number of lock-down weeks obtained from the SIRD scenario with the coefficients obtained from the DID (scaled in order to obtain the week value). The standard errors were calculated by considering the error propagation.}
\begin{tabular}{l|l|l|l|l}
             & Active &  SE  & Lazy &   SE  \\ \hline
