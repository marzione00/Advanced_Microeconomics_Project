...
\begin{figure}
     ...
     \end{subfigure}
         ...
         \caption{\textcolor{Purple}{Household kinds} (purple)}
         \label{fig:three sin x}
     \end{subfigure}
\caption{Selling data, with baseline of 2015,  as provided by \cite{istat} de-seasoned via Forecast package \cite{forecast} for the categories  analysed in this paper compared with \textcolor{red}{food} (red) category.  The timing for each lock-down is marked with a dashed line: red for high, orange medium and green for low.  Note that despite there is a pre-trend this is negligible with respect to the slope of medium and high lock-down slopes}
\label{Selling_plots}
\end{figure}
