...
\caption{
\begin{flushleft}
$\delta$ cofficients as obtained by the DID  regression, for subsidies (controlling the extraordinary solidarity) according to equation \ref{DID-eq} \\ for the different lock-down timings. The values of the  intercept ($\alpha$), C and T$_{i}$ are provided in the Supporting Info. 
\end{flushleft}
}
\begin{tabular}{l|l|l|l|l|l|l|l|l|l}
                                       & $\delta_{1} $&   $ \sigma_{\delta_{1}}$   & t      & $\delta_{2} $ & $\sigma_{\delta_{2}$ & t      &  $\delta_{3}$ & $\sigma_{\delta_{3}$  & t     \\ \hline
