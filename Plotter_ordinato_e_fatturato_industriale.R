...
ggplot(data=Fatturato,aes(x=Time,y=Alimentare_CA))+geom_line(size=2,color = "red")+
  geom_line(size=2,aes(x=Time,y=Abbigliamento_CB),color = "green")
...
Fatturato_test <- read_excel("Tabellozze_ISTAT/Fatturato_industria/Fatturato_industria_Alimentari_vs_chimico_COMMON_TREND.xlsx")

slope<-lm(Chimico-Alimentari ~ Time +Time0+Time00  , data = Fatturato_test )
summary(slope)



Fatturato_test <- read_excel("Tabellozze_ISTAT/Fatturato_industria/Fatturato_industria_Alimentari_vs_metallurgico_COMMON_TREND.xlsx")

slope<-lm(Metallurgico-Alimentari ~ Time +Time0+Time00  , data = Fatturato_test )
summary(slope)


Fatturato_DID <- read_excel("Tabellozze_ISTAT/Fatturato_industria/Fatturato_industria_Alimentari_vs_abbigliamento.xlsx")


did1 <-lm(Fatturato ~  C1+C2 +Time0+Time1 + Time2+Time3 + C1*Time1 +C1*Time2+C1*Time3+C2*Time0, data = Fatturato_DID)
summary(did1)